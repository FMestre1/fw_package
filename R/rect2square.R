  ...
  suppressWarnings(
    ...
    for(i in 1:length(list1)){
      ...
      if (ncol(m1)!=nrow(m1) | !setequal(rnames,cnames)){
        ...
        col_names0 <- str_replace_all(col_names0,"\\."," ")
        row_names0 <- str_replace_all(row_names0,"\\."," ")
        col_names0 <- str_replace_all(col_names0,"-"," ")#remove dashes
        row_names0 <- str_replace_all(row_names0,"-"," ")#remove dashes
        ...
        for(a in 1:size_M){
          for(b in 1:size_M){
            row1 <- all_names[a]
            col1 <- all_names[b]
            row1 <- which(row_names0==row1)
            col1 <- which(col_names0==col1)
            el1 <- as.numeric(as.character(m1[row1,col1]))
            if(length(el1)==0) el1 <- 0
            if(is.na(el1)) el1 <- 0
            m2[a,b] <- el1
          }
        }
